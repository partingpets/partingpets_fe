...
class Payments extends React.Component {
  ...
  render() {
    ...
    const {
      calledPaymentOptionsInState, selectedPaymentId, isEditingPayment, paymentIdToEdit,
    } = this.state;
    ...
    const paymentSelector = paymentOption => (
      <option key={paymentOption.id} paymentoption={paymentOption} value={paymentOption.id}>
        {paymentOption.name}
      </option>
    );
    ...
    const listOrSelector = () => {
      if (isProfilePage) {
        return <div>{listedPaymentOptions}</div>;
      } return (
          <Form>
            <Row>
              <Col md={3}>
                <FormGroup>
                  ...
                  <Input
                    onChange={this.selectedPaymentChange}
                    type="select"
                    name="select"
                    id="select"
